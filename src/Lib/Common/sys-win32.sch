...
;
; $Id: sys-win32.sch 5873 2008-12-21 21:44:05Z will $
;
; Win32 operating system hooks: safe interfaces to unsafe primitives.
; Based on Unix, for now.
...
; the parent process.  Later, we open CON:
    ...
    ((input) 
     (if *conio-input-firsttime* 
         (begin (set! *conio-input-firsttime* #f)
                unix:stdin)
         (unix:open "CON:" unix:open-read 0)))
    ((output) 
