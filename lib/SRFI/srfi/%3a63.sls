;;; SRFI 47: Array
;;;
;;; $Id: %3a63.sls 6149 2009-03-19 02:41:56Z will $
;;;
;;; Conflicts with (rnrs base): equal?
;;;
;;; Modifications by Will Clinger are marked by ;[wdc].
...
;[wdc] All R6RS syntax was introduced by Will Clinger.

(library (srfi :63 arrays)
  ...
  (import (except (rnrs base) equal?)
          ...
          (rnrs r5rs)
          (rename (srfi :99 records procedural)
                  (rtd-predicate record-predicate)
                  (rtd-accessor record-accessor)))

;;@code{(require 'array)} or @code{(require 'srfi-63)}
;;@ftindex array

;(require 'record)              ;[wdc]
;(require 'multiarg-apply)      ;[wdc]

(define (slib:error who . rest)                             ;[wdc]
  (apply error who "error detected by SRFI 63" rest))       ;[wdc]

(define (make-record-type name fields)                      ;[wdc]
  (make-rtd (string->symbol name) (list->vector fields)))   ;[wdc]

(define (record-constructor rtd fields)                     ;[wdc]
  (rtd-constructor rtd (list->vector fields)))              ;[wdc]
