...
;;;
;;; $Id: %3a43.sls 6152 2009-03-19 22:30:05Z will $
;;;
...
;;;
;;; NOTE:  The conflicts imply that we can't use r5rs:require to get
;;; Larceny's implementation of the R5RS SRFI.
;;;
;;; FIXME: a lot of this code is unnecessarily inefficient.

(library (srfi :43 vectors helper)

  (export (rename (vector-fill! %vector-fill!)
...
(define vector-fill!
  (lambda (vec value . maybe-start+end)
    (if (null? maybe-start+end)
        (%vector-fill! vec value)     ; from (srfi :43 vectors helper)
...
(define vector->list
  (lambda (vec . maybe-start+end)
    (if (null? maybe-start+end)       ; Oughta use CASE-LAMBDA.
        (%vector->list vec)           ; from (srfi :43 vectors helper)
...
(define list->vector
  (lambda (lst . maybe-start+end)
    ...
    (if (null? maybe-start+end)       ; Oughta use CASE-LAMBDA.
        (%list->vector lst)           ; from (srfi :43 vectors helper)
