...
;
; $Id: bignums-be.sch 6131 2009-03-12 23:21:16Z will $
;
; Endianness-dependent code for bignum arithmetic: big-endian.
;
...
(let ()
  ...
  ; This has been manually in-lined below where it is used.
  ; The new compiler should be able to in-line it automatically.
...
(define (big-fits-in-fix? b)
  (let ((b0 (bytevector-like-ref b 3)))
    (cond ((= 1 b0)
           (and (= 0 (bytevector-like-ref b 2))
                (= 0 (bytevector-like-ref b 1))
                (let* ((hi8 (bytevector-like-ref b 4))
                       (hi3 (fxlogand #xe0 hi8)))
                  (or (= 0 hi3)
                      (and (= negative-sign (bytevector-like-ref b 0))
                           (= #b00100000 hi8)                       ; FIXME
                           (= 0 (bytevector-like-ref b 5))
                           (= 0 (bytevector-like-ref b 6))
                           (= 0 (bytevector-like-ref b 7)))))))
          ((= 0 b0)
           (and (= 0 (bytevector-like-ref b 2))
                (= 0 (bytevector-like-ref b 1))))
          (else
...
(define (bignum-length32 b)
  (+ (bytevector-like-ref b 3)
     (fxlsh (bytevector-like-ref b 2) 8)
     (fxlsh (bytevector-like-ref b 1) 16)))
...
(define (bignum-length32-set! b l)
  (let* ((b0 (fxlogand l #xff))
         ...
         (b1 (fxlogand t1 #xff))
         (b2 (fxrshl t1 8)))
    (bytevector-like-set! b 3 b0)
    (bytevector-like-set! b 2 b1)
    (bytevector-like-set! b 1 b2)))
...
(define (bignum-shift-left! b c k)
  ...
  ; On little-endian machines, the arguments to the main loops
  ; are bytevector indexes.  That is preserved here, but results
  ; in some bit twiddling on big-endian machines:
  ; (+ i (- 3 (fxlogand 3 i))) converts a little-endian byte
  ; index into a big-endian byte index (assuming 32-bit bigits).
  ;
  ; FIXME: calls to this procedure should be inlined.
  ; In some of the more important loops, however, the order
  ; of bytes within a word doesn't matter, or the calls have
  ...
  (let* ((n (bignum-length32 b))        ; 32-bit bigits
    ...
    (define (slow-loop i)
      (if (>= i nbytes)
          (zero-loop (max 4 (- i k8)))
          (let ((b0 (bytevector-like-ref b (be i)))
                (c1 (bytevector-like-ref c (be (- (- i k8) 1))))
                (mask (fxrshl #xff k8bits)))
            (bytevector-like-set! c (be (- i k8)) (fxrshl b0 k8bits))
            (bytevector-like-set! c
                                  (be (- (- i k8) 1))
                                  (fxlogior (fxlogand c1 mask)
                                            (fxlsh b0 (- 8 k8bits))))
    ...
    (cond ((< k 0)
          ...
          ((= 0 k8bits)
           (fast-byte-loop (+ 4 k8)))
          ((< (+ 4 k8) nbytes)
           (let ((b0 (bytevector-like-ref b (be (+ 4 k8)))))
              (bytevector-like-set! c
                                    (be 4)
...
(define (bignum-add! b c i j)
  (let* ((n (bignum-length32 b)) ; n 16-bit bigits
    ...
    ;; j is a multiple of 4, and carry is 0 or 1.

    (define (carry-loop c j carry)
      (cond ((= 0 carry)
             c)
            (else
             (let* ((cj0 (bytevector-like-ref c (+ j 3)))
                    (cj1 (bytevector-like-ref c (+ j 2)))
                    (cj2 (bytevector-like-ref c (+ j 1)))
                    (cj3 (bytevector-like-ref c j))
               ...
               (bytevector-like-set! c (+ j 3) cj0)
               (bytevector-like-set! c (+ j 2) cj1)
               (bytevector-like-set! c (+ j 1) cj2)
               (bytevector-like-set! c j cj3)
               (carry-loop c (+ j 4) (fxrshl t3 8))))))
    ...
    (define (loop b c i j nbytes carry)
      (if debugging
          (begin (write b)
                 ...
                 (newline)))
      (cond ((>= i nbytes)
             (carry-loop c j carry))
            (else
             (let* ((bi0 (bytevector-like-ref b (+ i 3)))
                    (bi1 (bytevector-like-ref b (+ i 2)))
                    (bi2 (bytevector-like-ref b (+ i 1)))
                    (bi3 (bytevector-like-ref b i))

                    (blo16 (fxlogior (fxlsh bi1 8) bi0))
                    (bhi16 (fxlogior (fxlsh bi3 8) bi2))

                    (cj0 (bytevector-like-ref c (+ j 3)))
                    (cj1 (bytevector-like-ref c (+ j 2)))
                    (cj2 (bytevector-like-ref c (+ j 1)))
                    (cj3 (bytevector-like-ref c j))
...
(define (bignum-subtract! b c i j)
  (let* ((n (bignum-length32 b)) ; n 16-bit bigits
    ...
    ;; j is a multiple of 4, and borrow is 0 or 1.

    (define (borrow-loop c j borrow)
      (cond ((= 0 borrow)
             c)
            (else
             (let* ((cj0 (bytevector-like-ref c (+ j 3)))
...
(define (bignum-multiply-by-scalar-and-add! b c i j khi klo)
  (let* ((n (bignum-length32 b)) ; n 32-bit digits
    ...
    (define (carry-loop c j carry-hi carry-lo)
      (cond ((and (= 0 carry-hi) (= 0 carry-lo))
             c)
            (else
             (let* ((cj0 (bytevector-like-ref c (+ j 3)))
                    (cj1 (bytevector-like-ref c (+ j 2)))
               ...
               (bytevector-like-set! c (+ j 1) cj2)
               (bytevector-like-set! c j cj3)
               (carry-loop c (+ j 4) 0 (fxrshl t3 8))))))
