...
;;;
;;; $Id: srfi-14.sch 2926 2006-05-01 23:14:51Z pnkfelix $
;;;
;;; This is the reference implementation, adapted to Larceny.  See
;;; below for porting notes.
;;;
;;; Larceny has bytevectors and bit operations; using these to
;;; represent character sets would likely reduce both space and time.
...
;  - included other SRFIs as appropriate
;  - rewrote LET-OPTIONALS* and :OPTIONAL
;  - rewrote BITWISE-AND as LOGAND
;  - made a check-arg macro that removes the test, since this code
;    will generally be compiled "safe"
;  - latin->char and char->latin are integer->char and char->integer

(cond-expand ((and srfi-9		; records
		   srfi-23		; ERROR
		   larceny		; LOGAND, character representation
		   )))
...
(define (ucs-range->char-set lower upper . rest)
  (let ((error? (if (null? rest) #f (car rest)))
	(bs (%default-base (if (pair? rest) (cdr rest) '()) ucs-range->char-set)))
